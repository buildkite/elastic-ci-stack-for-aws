...
echo "TODO: Fix figuring out what the latest agent version of this type is"
exit 1
...
if ! aws s3 cp "s3://${BUILDKITE_AWS_STACK_BUCKET}/${AGENT_RELEASE_STREAM}/${packer_file}" . ; then
  make build-ami | tee "$packer_file"
  aws s3 cp "${packer_file}" "s3://${BUILDKITE_AWS_STACK_BUCKET}/${AGENT_RELEASE_STREAM}/${packer_file}"
else
